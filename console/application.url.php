...
if ($_SERVER['REQUEST_METHOD'] == 'POST') 
{
    ...
    if (
        ...
        validate_url_exists($_POST['url'], 'applications', $_application['id']))
    {
        die('here');
        message_set('URL Error', 'There was an error with your application URL.', 'red');
        header_redirect('/application/url');
    }
    ...
    message_set('Password Success', 'Your application URL has been updated.');
...
<?php if(!$_application['url']): ?>
    <p>
        Adding a URL to your application profile will make your application publicly visable at:
        <br />
        <a href="#"><?=ENV_CONSOLE_DOMAIN?>/profile/<span id="application-url">&lt;CITY_URL&gt;</span></a>
    </p>
<?php else: ?>
    <p>
        Your application profile is currently available at:
        <br />
        <a href="<?=ENV_CONSOLE_DOMAIN?>/profile/<?=$_application['url']?>"><?=ENV_CONSOLE_DOMAIN?>/profile/<span id="your-url"><?=$_application['url']?></span></a>
    </p>
    <p>
        Changing your application URL will cause your previous URL to no longer function and it will become available
